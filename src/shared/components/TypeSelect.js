...
export default function TypeSelect({ allTypes, store }) {
  ...
  return (
    ...
    >
      <div
        className='flex h-9 items-center justify-between gap-4 rounded-lg border-2 bg-white px-3 outline-none focus:border-blue-500'
        tabIndex={0}
      >
          ...
          <div className='flex gap-1'>
            {state.typeFilter.map((type) => (
              <TypeButton
                ...
                onClick={() => {
                  dispatch({
                    typeFilter: state.typeFilter.filter((t) => t !== type),
                  });
                }}
              />
            ))}
