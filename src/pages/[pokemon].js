...
import TypeButton from '../shared/components/TypeButton';

import { BASE_URL } from '../pages/index';
import { IMAGES_URL } from '../shared/components/PokemonCard';
...
import { IMAGES_URL_OF } from '../shared/config/api';
...
export default function Pokemon(props) {
  ...
  return (
    <div
      className={`flex min-h-screen flex-col gap-4 pb-10 ${
    ...
    >
      ...
      >
        ...
        <p className='font-bold'># {`${id}`.padStart(4, '0')}</p>
      </header>
        ...
        {previousPokemon > 0 && (
          <Link href={`/${previousPokemon}`}>
            <a>
              <LeftArrow
                size={30}
                className='absolute left-2 top-[50%] cursor-pointer'
              />
            </a>
          </Link>
        )}
          ...
          <Link key={type} href='/'>
            <a>
              <TypeButton
                ...
                onClick={() =>
                  dispatch({ typeFilter: [type], searchFilter: '' })
                }
              />
            </a>
          </Link>
          ...
          {evolutionChain.map((pokemon) => (
            <Link key={pokemon.name} href={`/${pokemon.name}`}>
              <div className='flex cursor-pointer flex-col gap-2'>
                <Image
                  src={`${IMAGES_URL_OF}${pokemon.id}.png`}
                  alt={pokemon.name}
                  width='150'
                  height='150'
...
export async function getServerSideProps({ params: { pokemon } }) {
  try {
    const pokemonData = await fetch(`${BASE_URL}/pokemon/${pokemon}`).then(
      (res) => res.json()
    );
    ...
    const varieties = speciesData.varieties
      ...
      .filter(
        ({ name }) =>
          evolutionChain.every((pokemon) => pokemon.name !== name && name) &&
          name !== pokemon
      );
